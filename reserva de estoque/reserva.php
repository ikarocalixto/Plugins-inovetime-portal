...
function user_expense_panel_shortcode() {
    ...
    if (($current_day == 2 || $current_day == 3|| $current_day == 4|| $current_day == 5|| $current_day == 6|| $current_day == 7|| $current_day == 10|| $current_day == 11|| $current_day == 12|| $current_day == 13|| $current_day == 14||$current_day == 15|| $current_day == 16|| $current_day == 17|| $current_day == 18|| $current_day == 19|| $current_day == 20|| $current_day == 1) && $total_expense > 0) {
...
function create_automatic_product($user_id, $amount, $type = 'main_invoice') {
    ...
    $current_time = time();
// Recupera os detalhes da despesa
    $expense_details_array = get_user_meta($user_id, 'expense_details', true);
    $expense_details = '';
    if (is_array($expense_details_array)) {
        // Se os detalhes da despesa são um array, converta para uma string formatada
        foreach ($expense_details_array as $detail) {
            $expense_details .= $detail . '; '; // Separe cada detalhe com ponto e vírgula
        }
        $expense_details = rtrim($expense_details, '; '); // Remove o último ponto e vírgula
    } else {
        // Se não for um array, simplesmente converte para string
        $expense_details = (string) $expense_details_array;
    }
    
    // O conteúdo do post que inclui os detalhes da despesa
    $post_content = 'Detalhes da Despesa: ' . $expense_details;
...
function render_expense_admin_page() {
 ...
 // quadro kaban 
    ...
    // No início do seu arquivo PHP, após buscar os usuários
$users = get_users();
$total_expense_all_users = 0; // Inicializa a variável para armazenar a soma das despesas

// Depois disso, você pode acumular o total das despesas de todos os usuários em um loop
foreach ($users as $user) {
    $total_expense = get_user_meta($user->ID, 'total_expense', true);
    $total_expense_all_users += (int)$total_expense; // Converte o valor para inteiro e soma ao total
}

// Depois, você pode exibir o total em algum lugar no seu HTML
?>


<h2>Total de Fatura de Todos os Franqueados: <?php echo $total_expense_all_users_formatted = 'R$ ' . number_format($total_expense_all_users, 2, ',', '.'); ?></h2>
            ...
            <?php foreach ($users as $user): ?>
                ...
                <?php if ($total_expense > 0 && $total_expense <= 1000): ?>
                    <div class="user-card" data-user-id="<?php echo $user->ID; ?>">
                        ...
                        <p>Fatura: <?php echo $total_expense =  'R$ '. number_format($total_expense, 2, ',', '.'); ?></p>
                    </div>
                <?php endif; ?>
            <?php endforeach; ?>
        </div>
    ...
    <div id="dialog" title="Fatura detalhada de produtos para reposição de estoque ">
...
// Verificar se o formulário de limpeza foi enviado
    ...
    <h2>Remover a Fatura total de um usuário</h2>
    <form method="post" action="">
        ...
        <input type="submit" value="Remover todos as Vendas">
    </form>
    <?php
...
if (isset($_POST['remove_user_id']) && isset($_POST['remove_amount'])) {
    ...
    ?>
  <h2>Adicionar uma fatura ao franqueado</h2>
<form method="post" action="">
    <label for="user_id">Usuário:</label>
    <select id="user_id" name="user_id" required>
        <?php foreach ($users as $user): ?>
...
<h2>Remover fatura do usuário</h2>
<form method="post" action="">
...
<h2>Fatura detalhada dos Franqueados:</h2>
...
function user_expense_details_table($user_id) {
    ...
    if (is_array($expense_details) && count($expense_details) > 0) {
        ob_start();
        ?>
        <h2>Fatura detalhada de produtos para reposição de estoque </h2>
