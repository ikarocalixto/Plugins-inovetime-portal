...
function pagina_importacao_xml() {
    ...
    if (isset($_POST['importar_produto_especifico'])) {
     ...
     // Verifica se o usuário acionou a importação de todos os produtos
if (isset($_POST['processar_importacao_todos_produtos'])) {
    processar_importacao_todos_produtos();
    echo '<div class="notice notice-success"><p>Todos os produtos foram importados com sucesso!</p></div>';
}
...
echo "</form>";
             // Dentro da sua função `pagina_importacao_xml`
             echo "<form method='post' action='' id='form-importar-produto2'>";
             echo "<input type='hidden' name='produto_id' value='{$produto_id}'>";
             echo "<input type='submit' name='processar_importacao_todos_produtos' value='IMPORTAR TODOS ProdutoS!' id='processar_importacao_todos_produtos'>";
             echo "</form>";
...
function processar_importacao_todos_produtos() {
    ...
    $xml->registerXPathNamespace('g', 'http://base.google.com/ns/1.0');
    // Ajuste a expressão XPath conforme necessário para corresponder à estrutura do seu XML
    $produtos = $xml->xpath("//channel/item");

    if (empty($produtos)) {
        echo "<div><p>Nenhum produto encontrado no XML.</p></div>";
        return;
    }

    foreach ($produtos as $item) {
        $produto_id = sanitize_text_field((string)$item->id); // Assumindo que cada produto tem um ID único no XML

        // Verifica se o produto já existe (pode ser necessário ajustar esta função)
...
$produto->set_stock_status('outofstock'); // Define como indisponível
  ...
  echo "<div><p> TODOS OS PRODUTOS '{$produto->get_name()}' importado com sucesso.</p></div>";
}
}
